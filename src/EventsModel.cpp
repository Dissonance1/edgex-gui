...
QVariant EventsModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        ...
        switch (index.column()) {
            ...
            case 1: {
                QString name = event["deviceName"].toString();
                if (name.isEmpty()) name = event["profileName"].toString();
                return name;
            }
            ...
            case 3: {
                ...
                // EdgeX timestamps can be Milli (13 digits) or Nano (19 digits)
                if (ts > 1000000000000000LL) ts /= 1000000; 
                return QDateTime::fromMSecsSinceEpoch(ts).toOffsetFromUtc(19800).toString("yyyy-MM-dd HH:mm:ss");
            }
            case 4: {
